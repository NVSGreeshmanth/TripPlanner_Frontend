...
import {
  _ovDismiss, _ovOpen, _svToggleLeg, closeStopsView, openStopsView, refreshStopsView,
...
function _installWindowBridge() {
  Object.assign(globalThis, {
    ...
    _navTab, _svToggleLeg, clearDataSheet, clearInput, closeFavRes, closeJourneyView,
    ...
    addFavJ, removeFavJ, untrackFromHome,
...
const _vpOccToSeat = { many:'empty', few:'few', standing:'few', crowded:'full', full:'full', closed:'full' };
...
function renderJourneys(data, _pastData, doScroll = false, showAll = false) {
  ...
  const _buildOneCard = (j, idx, animate) => {
    ...
    return { uid, html: buildJCard(uid, dep, arr, firstTL, lastTL, origPlat, destPlat, legs, badges, xfers, animate, idx, fareHtml) };
  };
...
function buildJCard(uid, dep, arr, firstTL, lastTL, origPlat, destPlat, legs, badges, xfers, animate, animIdx = 0, fareHtml = '') {
  ...
  const bestDelay  = Math.min(depDelay, arrDelay);
  let statusPill = '';
  if (worstDelay > 0) {
    statusPill = `<span class="status-pill status-delayed">${t('delayed_badge').replace('{n}', worstDelay)}</span>`;
  } else if (bestDelay < 0) {
    statusPill = `<span class="status-pill status-early">${t('early_badge').replace('{n}', Math.abs(bestDelay))}</span>`;
  } else if (depPlanned || arrPlanned) {
    statusPill = `<span class="status-pill status-ontime">${t('ontime_badge')}</span>`;
  }
  ...
  //  • Completed (already arrived) → "✓ Completed" — realtime has expired, so it's
  //    NOT live and there's no delay to show (only planned times remain).
  //  • In progress / upcoming with realtime → "● Live".
  //  • Otherwise → "No live data" (scheduled times only).
  ...
  const rtBadge = !_transitLegs.length ? ''
    : _journeyDone
      ...
      : _transitLegs.some(legIsRealtime)
        ? `<span class="rt-badge rt-on" title="${t('rt_on_tip') || 'Real-time data available'}"><span class="rt-dot"></span>${t('rt_live') || 'Live'}</span>`
        : `<span class="rt-badge rt-off" title="${t('rt_off_tip') || 'No real-time data — scheduled times only'}">○ ${t('rt_none') || 'No live data'}</span>`;
  // "Completed" already conveys it departed — drop the redundant "Departed" pill.
  if (_journeyDone) departedPill = '';

  // Live carriage count (from GTFS-RT consist) for the boarding service
  const _liveV = (() => {
    const l = firstTransit || legs[0];
    const mot = l?.transportation?.product?.class;
    if (mot == null) return null;
    return _vpByTripId(_vpTripId(l), mot)
        || _vpByRouteName(l.transportation?.disassembledName || l.transportation?.number, mot);
  })();
  const carsBadge = _liveV?.carriages
    ? `<span class="cars-badge" title="${_liveV.model ? _liveV.model + ' · ' : ''}${_liveV.carriages}-carriage set">🚃 ${_liveV.carriages} ${t('cars')}</span>` : '';

  // "Departs in X min" countdown shown when the boarding service hasn't left yet.
  // Walk-only journeys have no transit leg, so they never show a departure badge.
  let departsIn = '';
  if (!hasDeparted && firstTransit && transitDep) {
    const mins = Math.round((new Date(transitDep) - nowCheck) / 60000);
    if (mins >= 0 && mins <= 90) {
      const cls = mins <= 1 ? 'dep-in-now' : mins <= 5 ? 'dep-in-soon' : 'dep-in-ok';
      const txt = mins === 0
        ? t('departing_now')
        : `${t('departs_in')} <strong>${mins}</strong> ${t('min')}`;
      departsIn = `<span class="dep-in-badge ${cls}" title="${t('departs_in_tip')}">🕒 ${txt}</span>`;
    }
  }
  ...
  const expandLabel = stopCount > 2
    ? `<span class="jexpand-main">${t('view_details')}</span><span class="jexpand-sub">${stopCount} ${t('stops_word')} · ${t('live_word')}</span>`
    : `<span class="jexpand-main">${t('view_details')}</span>`;
  ...
  return `<div class="jcard ${delayClass}${tracked ? ' jcard-tracked' : ''} fadein" style="${anim}" data-juid="${uid}">
    ...
    <div class="jcard-timeline" onclick="openStopsView('${uid}')">
      <div class="jcard-time-col">
        <div class="jtime-row">${depTimeHtml}${delayBadge(depDelay)}</div>
        ${plannedLine(depPlanned, dep)}
      </div>
      <div class="jcard-dur-line">
        <div class="jcard-dur-bar"></div>
        <div class="jcard-dur-pill">${dur(dep, arr)}</div>
      </div>
      <div class="jcard-time-col jcard-time-right">
        <div class="jtime-row jtime-row-right">${delayBadge(arrDelay)}${arrTimeHtml}</div>
        ${plannedLine(arrPlanned, arr)}
      </div>
    </div>

    <!-- ② Station names (prominent) -->
    <div class="jcard-stations" onclick="openStopsView('${uid}')">
      <div class="jstation">${origName}</div>
      <div class="jstation jstation-right">${destName}</div>
    </div>

    <!-- ③ Route badges + transfers -->
    <div class="jcard-row2" onclick="openStopsView('${uid}')">${badges}${xferBadge ? `<span style="margin-left:2px"></span>${xferBadge}` : ''}</div>

    <!-- ④ Slim meta: status · seats · accessibility · countdown. Car count and
         platforms live in the details view; accessibility stays here because it's
         essential info, not clutter. -->
    <div class="jcard-meta-row">${trackingPill || departedPill || statusPill || ''}${rtBadge}${seat}${a11y}${departsIn ? `<span class="jcard-meta-spacer"></span>${departsIn}` : ''}</div>

    ${fareHtml}
    <button class="jexpand-btn" onclick="openStopsView('${uid}')" aria-label="${t('view_details')}">${expandLabel}<span class="jexpand-chev" aria-hidden="true">›</span></button>
  </div>`;
}
