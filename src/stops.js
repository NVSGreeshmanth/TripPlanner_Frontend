...
// UI-initiated close: route through history so the Back stack stays in sync.
...
export function _ovDismiss(key) {
  ...
  history.go(-(navState.ovStack.length - idx));              // pops this + any above it
  return true;
}
window.addEventListener('popstate', () => {
  const top = navState.ovStack.pop();
...
export async function _svToggleLeg(key) {
  ...
  _ovOpen('leg', () => _svCollapseLeg(key));        // Back returns to the overview
...
export function _svScrollToLegBoard(key) {
  ...
  requestAnimationFrame(() => {
    const board = scroller.querySelector(`[data-board="${key}"]`);
    if (!board) return;
    const head  = scroller.querySelector(`[data-leghead="${key}"]`);
...
export function _svScrollToBoard() {
  ...
  const board = body.querySelector('.sv-stop-from');
  if (!board) return;
...
export function showToast(msg) {
  ...
  // Give longer messages (e.g. the "already tracking" warning) more reading time.
  _toastTimer = setTimeout(() => el.classList.remove('show'), msg.length > 40 ? 3200 : 1800);
}
...
export function _buildLegSection(leg, legIdx, entry, loading, allLegs, focusMode = false) {
  ...
  // Service header — show all available live data as feature chips
  const featureChips = [];
  if (liveVehicle?.model) {
    ...
    featureChips.push(`<span class="sv-feat">🚆 ${_modelTxt}${_isTrain ? '-set' : ''}</span>`);
  }
  if (liveVehicle?.carriages)  featureChips.push(`<span class="sv-feat">${liveVehicle.carriages} cars</span>`);
  if (liveVehicle?.air_con)    featureChips.push(`<span class="sv-feat">❄ A/C</span>`);
  if (liveVehicle?.accessible) featureChips.push(`<span class="sv-feat">♿ Accessible</span>`);
  // Live occupancy (works for ferry / light rail / bus when the feed provides it)
  if (liveVehicle?.occ) {
    const _OCC_CHIP = {
      empty:    ['🟢', t('occ_seats')], many: ['🟢', t('occ_seats')],
      few:      ['🟡', t('occ_few')],   standing: ['🟡', t('occ_standing')],
      crowded:  ['🔴', t('occ_crowded')], full: ['🔴', t('occ_full')],
    };
    const oc = _OCC_CHIP[liveVehicle.occ];
    if (oc) featureChips.push(`<span class="sv-feat">${oc[0]} ${oc[1]}</span>`);
  }
  if (liveVehicle?.prior_trip) featureChips.push(`<span class="sv-feat sv-feat-warn">⚠ Prior trip</span>`);
  ...
  // Delay / early / on-time pill for this leg. Uses the SAME estimated times the
  // card/stop rows display (trip-planner estimate), so the pill, the leg card
  // times, the journey card and the board/alight stop rows all agree — no
  // mismatch from mixing the trip-planner estimate with the GTFS-RT feed.
  const _legDelay = delayMins(_legArrPlan || _legDepPlan, _hArr || _hDep);
  const _hasPlan = !!(_legDepPlan || _legArrPlan);
  let _statusPill = '';
  if (_legDelay > 0)
    _statusPill = `<span class="sv-leg-status sv-leg-late">${t('delayed_badge').replace('{n}', _legDelay)}</span>`;
  else if (_legDelay < 0)
    _statusPill = `<span class="sv-leg-status sv-leg-early">${t('early_badge').replace('{n}', Math.abs(_legDelay))}</span>`;
  else if (_hasPlan)
    _statusPill = `<span class="sv-leg-status sv-leg-ontime">${t('ontime_badge')}</span>`;
  ...
  const _topRow = `<div class="sv-legc-top">
      ...
      ${expanded ? `<span class="sv-legc-back"><span class="sv-legc-back-ic">‹</span>${t('back') || 'Back'}</span>` : ''}
    </div>`;
  if (expanded) {
    html += `<button type="button" class="sv-legc sv-legc-slim sv-leg-btn sv-legc-open" style="--leg:${bg}" onclick="_svToggleLeg('${legKey}')" aria-expanded="true">
    ${_topRow}
  </button>`;
  } else {
    ...
    ${featureChips.length ? `<div class="sv-feat-row">${featureChips.join('')}</div>` : ''}
  ...
  html += `</div>`;  // /sv-leg-head (slim header + formation when expanded)
  ...
  // Expanded leg → full terminus-to-terminus service list.
  const visStart = 0;
  const visEnd   = stops.length - 1;
  ...
  stops.forEach((s, i) => {
    ...
    const sName  = cleanStationName(s.name);
    const plat   = s.plat || '';
    const occIco = { many:'🟢', few:'🟡', standing:'🟠', crowded:'🔴', full:'🔴' }[s.occ] || '';

    // ── Position marker between stops ────────────────────────────────────
    // Insert BEFORE the first stop the service hasn't reached yet — this now
    // also lands in the before-boarding region, so you can see the train is
    // (e.g.) still approaching Epping while you wait at Strathfield. Driven by
    // the timetable (vehicleStopIdx); GTFS-RT just confirms it's running.
    if (!vehicleShown && !hasArrived && i > 0 && !isPast && lastPassedIdx >= 0) {
      vehicleShown = true;
      const nextStop = sName;
      html += `<div class="sv-position-marker">
        <div class="sv-position-line" style="background:${bg}"></div>
        <div class="sv-position-content">
          <span class="sv-position-icon-sm">${vIcon}</span>
          <span class="sv-position-txt">${t('approaching').replace('{icon}', '').replace('{stop}', nextStop).trim()}</span>
          ${liveBadge}
        </div>
        <div class="sv-position-line" style="background:${bg}"></div>
      </div>`;
    ...
    const cls = [
      'sv-stop',
      isPast || isBeforeBoard  ? 'sv-stop-past' : '',
      isAfterAlight && !isTerm ? 'sv-stop-after' : '',
      isFrom ? 'sv-stop-from' : '',
      isTo   ? 'sv-stop-to'   : '',
      skipped ? 'sv-stop-skipped' : '',
    ].filter(Boolean).join(' ');

    const dotCls = isFrom ? 'sv-dot-from' : isTo ? 'sv-dot-to' : isTerm ? 'sv-dot-terminus' : '';

    // A skipped stop shows no delay/ETA — the train isn't stopping there.
    const timeHtml = skipped
      ? `<span class="sv-skip-tag">${t('not_stopping') || 'Not stopping'}</span>`
      : `${tt ? fmt(tt) : ''}${delay ? `<span class="sv-delay ${delay > 0 ? 'sv-delay-late' : 'sv-delay-early'}"> ${delay > 0 ? '+' : ''}${delay}m</span>` : ''}${occIco ? ` ${occIco}` : ''}`;

    html += `<div class="${cls}"${isFrom ? ` data-board="${legKey}"` : ''}>
      <div class="sv-stop-timeline">
        <div class="sv-stop-dot ${dotCls}"></div>
        ${i < stops.length - 1 || showPostGap ? `<div class="sv-stop-line" style="background:${(isPast||isBeforeBoard) ? 'var(--muted)' : bg};opacity:${(isPast||isBeforeBoard) ? '0.25' : isAfterAlight ? '0.35' : '0.55'}"></div>` : ''}
      </div>
      <div class="sv-stop-info">
        <div class="sv-stop-time">${timeHtml}</div>
        <div class="sv-stop-name-wrap">
          <span class="sv-stop-name">${sName}</span>
          ${plat ? `<span class="sv-stop-plat">${plat}</span>` : ''}
          ${isFrom ? `<span class="sv-from-label">${t('board_label')}</span>` : ''}
          ${isTo   ? `<span class="sv-to-label">${t('alight_label')}</span>` : ''}
        </div>
      </div>
    </div>`;
  });

  // Collapse this leg back to its button.
  html += `<button type="button" class="sv-more" onclick="_svToggleLeg('${legKey}')">
    <span class="sv-more-ico">▴</span> ${t('show_less') || 'Show less'}
  </button>`;
